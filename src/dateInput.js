...
export default class DateInput {
    static showPicker(inputElement) {
        Picker.attachTo(inputElement);
    }

    static createMutationObserver(dateInputElement) {
            ...
            const mutationObserver = new MutationObserver((mutations) => {
                mutations.forEach((mutation) => {
                    if (mutation.attributeName.indexOf('min') !== -1 || mutation.attributeName.indexOf('max') !== -1) {
                        inputElement.minDateAttribute = DateInput.returnSelectedMinDate(inputElement.htmlElement);
                        inputElement.maxDateAttribute = DateInput.returnSelectedMaxDate(inputElement.htmlElement);
                        inputElement.dateRange = DateInput.getDateRange(inputElement.minDateAttribute, inputElement.maxDateAttribute);
                    } else if (mutation.attributeName === 'lang') {
                        inputElement.locale = DateInput.returnSelectedLocale(inputElement.htmlElement);
                        inputElement.localeLabels = DateInput.getLocaleLabels(inputElement.locale);
                    } else if (mutation.attributeName === 'data-first-day') {
                        inputElement.firstDayOfWeek = DateInput.returnSelectedFirstDayOfWeek(inputElement.htmlElement);
    ...
    static setupValueProperties(dateInputElement) {
        const inputElement = dateInputElement;
    ...
    static setupKeyEvents(newElement) {
    ...
    static returnDateFormat(htmlElement) {
